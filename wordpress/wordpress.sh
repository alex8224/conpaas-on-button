...
IP=$(echo $MYSQL_IP|awk -F':' '{print $1}')
MYSQL_PORT=$(echo $MYSQL_IP|awk -F':' '{print $2}')

echo "match mysql connection string $IP $PORT"
...
OLD_PHP_IP=`echo "SELECT option_value FROM wp_options WHERE option_name='home';" | mysql -u mysqldb -h $IP -P $MYSQL_PORT --password='contrail123' wordpress | tail -n 1`
...
if [ "$?" != "0" ]; then 
    ...
    echo "UPDATE wp_posts SET post_content = REPLACE (post_content, '$OLD_PHP_IP', '$PHP_IP');" | mysql -u mysqldb -h $IP -P $MYSQL_PORT --password='contrail123' wordpress
fi
